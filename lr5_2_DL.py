...
passw=""
passw+=alphabet[(alphabet.index(thirdw[2]) + 1 % (len(alphabet) - 1))]
passw+=alphabet[(alphabet.index(secondw[-2]) - 1 % (len(alphabet) - 1))]
if len(thirdw)% 2 != 0:
    passw+=alphabet[(alphabet.index(thirdw[0]) + 1 % (len(alphabet) - 1))]
else:
...
passw+=alphabet[((len(firstw)+len(secondw)-3)% 26)]
print(passw)
